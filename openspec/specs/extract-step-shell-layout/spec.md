# extract-step-shell-layout

## Purpose

Extracts the shared visual shell (blob animations, glass card, header, footer with auto-save text and ContinueButton) from individual step pages into a reusable `StepLayout.astro` component. Step pages declare only their form content; StepLayout composes inside the base Layout, hosts StepGuard and ContinueButton, and enforces `StepPath` typing at compile time. The summary page remains unchanged, using Layout directly.

...
Provides a thin presentation wrapper in `src/components/atoms/` for the shadcn `Select` primitive components, mirroring the existing pattern for `Button`, `Input`, `Label`, `RadioGroup`, `RadioGroupItem`, and `Checkbox`. This centralises the import surface so that future re-skin or prop injection is a one-atom edit.
